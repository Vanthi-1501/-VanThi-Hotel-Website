...
import { User, Mail, Lock, Loader2, ArrowRight, BookOpen } from 'lucide-react';
    ...
    <div className="min-h-screen bg-luxury-cream flex items-center justify-center px-6 py-20">
      ...
      >
        <div className="bg-luxury-dark p-10 text-white text-center">
          <h2 className="text-3xl font-serif font-bold mb-2">Đăng Ký</h2>
          <p className="text-gray-400 text-sm">Trở thành thành viên của gia đình VanThi</p>
        </div>

        <form onSubmit={handleSubmit} className="p-10 space-y-4">
          <div className="space-y-1">
            <label className="text-[10px] uppercase font-bold tracking-widest text-gray-500">Họ và tên</label>
            <div className="relative">
              <BookOpen className="absolute left-4 top-1/2 -translate-y-1/2 text-luxury-gold" size={18} />
              <input 
                type="text" 
                required
          ...
          <button 
            type="submit"
            disabled={loading}
            className="w-full bg-luxury-dark hover:bg-black text-white font-bold py-4 rounded-xl transition-all shadow-lg flex items-center justify-center gap-2 group mt-4"
          >
